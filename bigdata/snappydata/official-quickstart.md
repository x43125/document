# Structured Streaming use spark shell

## 1. Structured Streaming with socket
...
snappy.sql("create table if not exists test(id string, signal int) using column")

val schema = snappy.table("test").schema
...
val streamingQuery = df.
  ...
  writeStream.
  format("snappysink").
  queryName("testStream").  // must be unique across the TIBCO ComputeDB cluster
...
## 2. Structured Streaming with Kafka Source
