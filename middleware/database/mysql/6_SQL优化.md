...
1. 使用前导模糊搜索：like ‘%mysql’; like ‘%mysq%’

2. 在字段上进行计算或使用函数：where id + 1 < 10

3. 未命中最左前缀原则：使用联合索引的时候，要从左向后过滤（顺序无所谓）如：联合索引：`username, pwd` 
...
4. 类型转换：where name=1234 应该改为 where name=‘1234’

5. 如果做join操作，两表的join字段类型也要相同，否则不会走索引
...
6. 如果是 深度limit 分页，可以先用 where 条件过滤，如 limit 10000, 100  可以改成：where id>=10000 limit 0, 100

## 宏观建议

1. 使用EXPLAIN分析
2. 使用慢查询日志分析
3. 减少请求的数据量，减少返回的字段数，尽量索引覆盖
4. 减少服务器端扫描的行数
