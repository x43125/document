...
| 缩写 | 英文       | 中文   | 解释                                           | 实现           |
| ---- | ---------- | ------ | ---------------------------------------------- | -------------- |
| A    | Atomic     | 原子性 | 要么都成功，要么都失败                         | undo log       |
| C    | Consitency | 一致性 | 事务前后的数据状态一致，不会凭空增减           | A.I.D 共同实现 |
| I    | Isolation  | 隔离性 | 事务之间相互独立，不互相影响                   | 锁 + MVCC      |
| D    | Durability | 持久性 | 事务提交之后的数据永久保存，不会因为宕机等丢失 | redo log       |
...
| 隔离级别    | 解释                           | 问题                                                         | 实现                                                         |
| ----------- | ------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| RU 读未提交 | 可以读到其他事务未提交的数据   | 速度最快，但会脏读、不可重复读、幻读                         | 不加锁等直接读取                                             |
| RC 读已提交 | 可以读到其他事务已提交的数据   | 速度也快，但会不可重复读、幻读                               | 每次select 都生成最新数据的快照 快照读+行锁                  |
| RR 可重复读 | 多次读取同一个数据，会保持一致 | 速度略慢，但不会出现 不可重复读的情况，本身会出现幻读，但结合 间隙锁可以解决幻读 | select 数据只有在事务第一次的时候生成数据快照，之后的都复用 快照读 + MVCC + 间隙锁 |
| 序列化      | 所有请求按顺序读               | 速度最慢                                                     | 每个读都加锁                                                 |
